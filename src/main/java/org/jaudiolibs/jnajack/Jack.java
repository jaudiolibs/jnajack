...
public class Jack {
	...
	private void initCallbackMethods() {
		try {
			Class<?> ctiClass = Class.forName("com.sun.jna.CallbackThreadInitializer", true,
	 ...
	 * @throws net.neilcsmith.jnajack.JackException
	 */
	public String[] getPorts(JackClient client, String regex, JackPortType type, EnumSet<JackPortFlags> flags)
			throws JackException {
		...
		try {
			Pointer ptr = jackLib.jack_get_ports(client.clientPtr, null, typeString, new NativeLong(fl));
			if (ptr == null) {
				return new String[0];
			} else {
	 ...
	 * @author Matthew MacLeod
	 */
	public int getCurrentTransportFrame(JackClient client) throws JackException {
		try {
			return jackLib.jack_get_current_transport_frame(client.clientPtr);
		} catch (Throwable e) {
			LOG.log(Level.SEVERE, CALL_ERROR_MSG, e);
			throw new JackException(e);
		}
